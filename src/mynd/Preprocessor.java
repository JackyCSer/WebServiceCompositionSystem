...
public class Preprocessor
{
	 ...
	 */
	public static LinkedHashSet<Operator> preprocessForStrongCyclicPlanning(
			Set<Operator> ops)
	{
		// TODO: Prefer ops with less uncertainty, if an modified op is equal to
		// another op from ops.
		// TODO: Issue #45: Is this preprocessing useful because it leads to
		// biased costs in LAO*-search?
		LinkedHashSet<Operator> remainingOps = new LinkedHashSet<Operator>(
				(int) (ops.size() * 0.75) + 1);
		for (Operator op : ops)
		{
			...
			{
				if (newChoices.size() < explicitOp.getNondeterministicEffect()
						.size())
				{
					explicitOp.setNonDeterministicChoices(newChoices); // At
																		// least
																		// one
																		// empty
																		// effect
	...
	/**
	 * Remove operators with empty effects, because such an operator causes a
	 * self loop and can not be part of a strong solution.
	 * 
	 * @param ops
	 *            operators to be processed
	 * @return preprocessed operators
	 */
	public static LinkedHashSet<Operator> preprocessForStrongPlanning(
			Set<Operator> ops)
	{
		LinkedHashSet<Operator> remainingOps = new LinkedHashSet<Operator>(
				(int) (ops.size() * 0.75) + 1);
